...
void uvSphere(  std::vector<glm::vec3> & vertices,
    ...
    indices.resize(  2*slices*stacks*3);
    triangles.resize(2*slices*stacks);
    float du = 2*PI/slices;
    float dv = PI/stacks;
    unsigned int i,j;
    float u,v,x,y,z;
    ...
    for (i = 0; i <= stacks; i++) {
        ...
        for (j = 0; j <= slices; j++) {
            ...
            y = -sin(v);
            z = sin(u)*cos(v);
            vertices[index]    = radius*glm::vec3(x, y, z);
            normals[index]     = glm::vec3(x, y, z);
            texCoords[index++] = glm::vec2(j/(float)slices, i/(float)stacks);
        }
    }
    unsigned int k = 0;
    unsigned short _v1, _v2, _v3, _v4;
    index = 0;
    for (j = 0; j < stacks; j++) {
